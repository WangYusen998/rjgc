...
async function onExtend(id) {
  try {
    ...
    await booking.extendBooking(id)
  ...
  <section class="my-bookings-page ds-page">
    ...
    <section class="booking-grid" v-if="filteredBookings.length > 0">
      <article v-for="item in filteredBookings" :key="item.id" class="booking-card ds-panel">
        ...
        <p><b>Date:</b> {{ item.createdAt ? new Date(item.createdAt).toLocaleString() : '-' }}</p>
        ...
        <div class="actions">
          <button class="ds-btn ds-btn-secondary ds-btn-pill btn" @click="router.push(`/customer/history/${item.id}`)">
            Details
          </button>
