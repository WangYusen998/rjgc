...
export const returnZones = [
  { id: 'rz-01', name: '北门还车�?, latitude: 30.7689, longitude: 103.9843, radiusM: 220 },
  { id: 'rz-02', name: '图书馆还车区', latitude: 30.7648, longitude: 103.9848, radiusM: 240 },
  { id: 'rz-03', name: '南区生活广场还车�?, latitude: 30.7599, longitude: 103.9817, radiusM: 260 },
]
...
export function statusText(status) {
  const zh = { available: '可租', reserved: '已预�?, charging: '充电�?, maintenance: '维修�? }
...
export function bookingStatusText(status) {
  const zh = { ongoing: '进行�?, paid: '已完�?, returned: '已还�?, cancelled: '已取�?, overdue: '已超�? }
...
export function readStaff() {
  return (
    uni.getStorageSync(ADMIN_STAFF_KEY) || [
      { id: 'staff-a', name: '调度�?A', role: '车辆部署', task: '北门补车 3 �?, status: '执行�? },
      { id: 'staff-b', name: '调度�?B', role: '低电量回�?, task: '南区回收低电量车 2 �?, status: '待出�? },
      { id: 'staff-c', name: '门店 C', role: '门店检�?, task: '检查头盔与二维码贴�?, status: '已完�? },
    ]
  )
}
...
export function createIssue({ scooterId = '', type = '其他', message = '', priority = '�? }) {
...
export function saveProfile(profile) {
  ...
  if (session.account) {
    ...
    if (!nextAccount || nextAccount.length < 4) throw new Error('账号至少 4 �?)
    if (nextAccount !== currentAccount && users.some((user) => user.account === nextAccount)) {
      throw new Error('这个账号已经被使�?)
    }
...
export function registerUser({
  ...
  if (!normalized || normalized.length < 4) throw new Error('账号至少 4 �?)
  if (!password || password.length < 6) throw new Error('密码至少 6 �?)
  if (users.some((user) => user.account === normalized)) throw new Error('这个账号已经被注�?)
  if (phone && users.some((user) => user.phone === phone)) throw new Error('这个手机号已经被使用')
  if (email && users.some((user) => user.email === email)) throw new Error('这个邮箱已经被使�?)
  ...
  const user = {
    ...
    idNumber,
    country,
    paymentMethod: country === '英国' && cardLast4 ? '信用�? : bankCardLast4 ? '中国银行�? : '未绑�?,
    bankName,
    bankCardLast4,
    cardLast4,
    campus: '西南交通大学犀浦校�?,
...
export function recoverAccount({ phone, email }) {
  ...
  if (!user) throw new Error('没有找到匹配的账�?)
  return user.account
}

export function resetPassword({ account, contact, password }) {
  if (!password || password.length < 6) throw new Error('新密码至�?6 �?)
  let found = false
  ...
  if (!found) throw new Error('账号和手机号/邮箱不匹�?)
  writeUsers(users)
  return true
}
...
export function createBooking({
  ...
  const booking = {
    ...
    lastAction: paymentMethod
      ...
      : '等待用户付款与安全协议确认�?,
    unlockMessage: `通信模块已向后台发�?${scooter.id} 解锁指令`,
    total,
  }
...
export function extendBooking(id, minutes = 15) {
  ...
  return updateBooking(id, {
    ...
    lastAction: `已延�?${minutes} 分钟，追�?${extraFee} 元`,
  })
}
...
export function mockPayBooking(id, selectedMethod = '') {
  ...
  const method = selectedMethod || (user && user.country === '英国'
      ...
      : user && user.bankCardLast4
        ? `${user.bankName || '中国银行�?} ****${user.bankCardLast4}`
        : '模拟钱包')
  return updateBooking(id, {
    status: 'paid',
    paidAt: formatTime(new Date()),
...
export function finishReturn(id, payload = {}) {
  ...
  return updateBooking(id, {
    ...
    damageReport: payload.damageReport || '�?,
    batteryFee,
    overdueFee,
    dispatchFee,
...
export function loginAdmin({ account, password }) {
  ...
  if (!admin) throw new Error('管理员账号或密码不正�?)
...
export function adminModuleMeta() {
  ...
  return [
    { type: 'overview', title: '运营总览', desc: '系统使用情况、车辆可用率、订单概�?, count: data.usage.activeOrders },
    { type: 'users', title: '用户记录', desc: '注册账号、实�?银行卡信息、高频用�?, count: data.users.length },
    { type: 'bookings', title: '预订记录', desc: '当前订单、历史订单、延期取消与支付状�?, count: data.bookings.length },
    { type: 'revenue', title: '收入统计', desc: '总收入、已支付收入、超时费、电费和调度�?, count: data.finance.revenue },
    { type: 'pricing', title: '租赁价格配置', desc: '按车型配置分钟价、门店日租价和押�?, count: data.priceSettings.length },
    { type: 'issues', title: '问题反馈管理', desc: '查看用户问题、设置优先级、处理状�?, count: data.issues.length },
    { type: 'promotions', title: '折扣与促销', desc: '高频用户优惠、校园优惠、启停促销', count: data.promotions.length },
    { type: 'scooters', title: '滑板车监�?, desc: '车辆位置、状态、电量和通信状�?, count: data.scooters.length },
  ]
}
...
export function getApiBase() {
  return uni.getStorageSync(API_BASE_KEY) || DEFAULT_API_BASE
}
